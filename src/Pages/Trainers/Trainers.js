...
import AddTrainer from '@/components/AddTrainers/AddTrainers';
...
const Trainers = () => {
    ...
    const fetchData = useCallback(async (skip, limits) => {
        try {
            ...
            toast.error("Không thể tải dữ liệu huấn luyện viên. Vui lòng thử lại sau.");
            setData([]);
            setAllTrainers([]);
            setTotalData(0);
            setStartFrom(-1);
    ...
    const handleSearchData = () => {
        ...
        const filtered = allTrainers.filter(item => {
            ...
            const nameMatch = item.name ? item.name.toLowerCase().includes(search.toLowerCase()) : false;
            const phoneMatch = item.mobileNo ? item.mobileNo.toLowerCase().includes(search.toLowerCase()) : false;
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            toast.success(`Đã xóa huấn luyện viên ${trainerToDelete.name}`);
            await fetchData(0, 9);
            setCurrentPage(1);
            setIsSearchModeOn(false);
            setSearch("");
            ...
            toast.error(err.response?.data?.message || 'Không thể xóa huấn luyện viên');
        } finally {
        ...
        <div className={`min-h-screen p-6 lg:p-8 transition-colors duration-300 ${
            ...
            <div className="mb-8 animate-fade-in">
                <div className="flex items-center gap-3 mb-2">
                    <div className={`p-2 rounded-xl ${isDark ? 'bg-accent-500/20' : 'bg-accent-100'}`}>
                        <SportsIcon className="text-accent-500" sx={{ fontSize: 28 }} />
                    </div>
                ...
                <p className={`text-lg ${isDark ? 'text-stone-400' : 'text-stone-600'}`}>
                    Quản lý đội ngũ huấn luyện viên phòng gym
                </p>
            </div>
            ...
            <div className={`flex flex-wrap items-center gap-3 p-4 rounded-2xl mb-6 backdrop-blur-xl transition-colors ${
                ...
                <button 
                    ...
                    className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm transition-all duration-200 ${
                        isDark 
                            ...
                            : 'bg-accent-500 hover:bg-accent-600 text-white shadow-md shadow-accent-500/30'
                    }`}
                >
                ...
                <button 
                    onClick={() => navigate('/trainers/schedule-overview')}
                    className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm transition-all duration-200 ${
                        isDark 
            ...
            <div className={`flex flex-wrap items-center justify-between gap-4 mb-6 p-4 rounded-xl ${
                ...
                <div className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-stone-800'}`}>
                    Tổng: <span className="text-accent-500">{totalData}</span> huấn luyện viên
                </div>
                            ...
                            <button 
                                ...
                                className={`w-9 h-9 flex items-center justify-center rounded-lg transition-all duration-200 ${
                                    currentPage === 1
                                        ...
                                        : isDark ? 'bg-stone-700 hover:bg-accent-500 text-white' : 'bg-white hover:bg-accent-500 hover:text-white text-stone-700 shadow-sm'
                                }`}
                            >
            ...
            <div className={`p-6 rounded-2xl min-h-[500px] ${
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-16">
                        <div className="w-12 h-12 border-4 border-accent-500/30 border-t-accent-500 rounded-full animate-spin mb-4"></div>
                    ...
                    <div className="flex flex-col items-center justify-center py-16">
                        <div className={`w-20 h-20 rounded-full flex items-center justify-center mb-4 ${
                            ...
                            <SportsIcon className={isDark ? 'text-stone-500' : 'text-stone-400'} sx={{ fontSize: 40 }} />
                        </div>
                        ...
                        <p className={`text-sm ${isDark ? 'text-stone-500' : 'text-stone-500'}`}>
                            Nhấn "Thêm huấn luyện viên" để bắt đầu
                        </p>
                    </div>
                )}
            </div>
            ...
            {addTrainer && (
                <Modal 
                    header="Thêm Huấn Luyện Viên Mới" 
            ...
            {isDeleteDialogOpen && trainerToDelete && (
                <ConfirmDialog
                    title="Xác nhận xóa"
                    message={`Bạn có chắc chắn muốn xóa huấn luyện viên "${trainerToDelete.name}"?\nHành động này không thể hoàn tác.`}
                    confirmText="Xóa"
                    cancelText="Hủy"
